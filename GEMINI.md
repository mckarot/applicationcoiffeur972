...
Utilisation de Codebase Investigator : Pour les tâches d'ingénierie logicielle complexes (refactoring, analyse de code, implémentation de fonctionnalités), utiliser l'outil `codebase_investigator` pour une compréhension approfondie du code, de sa structure et de ses dépendances.
...
**Lisibilité en mode paysage** : Assurez-vous que toutes les pages sont bien lisibles, bien agencées et fonctionnelles en mode paysage, en adaptant la mise en page si nécessaire pour éviter les débordements et optimiser l'utilisation de l'espace.
...
Tests automatisés : Lorsque cela est pertinent, proposer des exemples de tests automatisés pour les fonctionnalités critiques. Expliquer l'importance des tests automatisés par rapport aux tests manuels, en mettant l'accent sur la non-régression et la détection précoce des bugs. si les tests echoue, ne le relance pas tout de suite mais explique moi le probleme.
...
Bonnes pratiques de codage : Éviter l'utilisation de BuildContext traversant des gaps async sans vérification mounted. Remplacer les API dépréciées dès qu'elles sont identifiées. Pour `Color.withOpacity()`, préférer `Color.withAlpha((255 * opacity).round())` pour éviter la perte de précision et les avertissements de dépréciation. Utiliser des widgets réutilisables et des extensions pour éviter la duplication de code.
